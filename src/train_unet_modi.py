...
from models.unet_modi import UNet_2
...
def train(epoch):
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        writer_half.add_scalar("train_loss for Unet_cut_half", loss.item(), (len(train_loader) * (epoch - 1) + batch_idx))  # 675*e+i
        writer_small.add_scalar("train_loss for Unet simplify input-output layers", loss_small.item(), (len(train_loader) * (epoch - 1) + batch_idx))  # 675*e+i
        ...
        if batch_idx % 20 == 0:
            ...
            model_small.eval()
            with torch.no_grad():
                val_losses, val_losses_small = 0.0, 0.0
                val_dice_coeff, val_dice_coeff_small = 0.0, 0.0
            ...
            writer_half.add_scalar("validation/dice_coeff", mean_dice_coeff, (len(train_loader) * (epoch - 1) + batch_idx))

            writer_small.add_scalar("validation/val_loss for simplify in-out layer", mean_val_loss_small, (len(train_loader) * (epoch - 1) + batch_idx))
            writer_small.add_scalar("validation/dice_coeff for simplify in-out layer", mean_dice_coeff_small, (len(train_loader) * (epoch - 1) + batch_idx))
...
if __name__ == "__main__":
    ...
    writer_small.add_graph(model, (dummy,))
